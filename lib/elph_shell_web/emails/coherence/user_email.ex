...
defmodule ElphShellWeb.Coherence.UserEmail do
  @moduledoc """
  ...
  * Account Unlock
  ...
  """
  use Phoenix.Swoosh,
    view: ElphShellWeb.Coherence.EmailView,
    layout: {ElphShellWeb.Coherence.LayoutView, :email}
  ...
  def password(user, url) do
    ...
    |> to(user_email(user))
    |> add_reply_to()
    |> subject("ElphShell - Reset password")
    |> render_body("password.html", %{url: url, name: user.name})
    |> log()
  end
  ...
  defp from_email do
    case Coherence.Config.email_from() do
      # Elixir_LS says this can't ever happen. Won't remove it just in case
      # nil ->
      #   Logger.error(
